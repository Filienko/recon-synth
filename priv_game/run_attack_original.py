...
def run_categorical_attack(data_name, synth_model, n_rows, k, scale_type, n_queries, num_categories, secret_attr, reps, n_procs, data_dir):
    """
    Run the categorical reconstruction attack on synthetic data
    """
    results = []
    ...
    for rep in range(reps):
        ...
        # Extract attributes and secret categorical values
        attrs, secret_cats = process_data_categorical(df, secret_attr)
        
        # Load or generate queries
        query_dir = f'{rep_dir}/queries/simple_cat/{k}way/'
        if os.path.exists(f'{query_dir}/queries.pkl.xz'):
            with lzma.open(f'{query_dir}/queries.pkl.xz', 'rb') as f:
                queries = pickle.load(f)
        else:
            # Generate queries for categorical target
            os.makedirs(query_dir, exist_ok=True)
            queries = gen_all_simple_kway_categorical(attrs, k, num_categories)
        ...
        est_categories, sol, success = categorical_l1_solve(A, synth_result, num_categories, procs=n_procs)
        
        # Evaluate accuracy
        true_category = secret_cats[user]
        predicted_category = est_categories[user]
